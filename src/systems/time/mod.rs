...
impl Plugin for DilationPLugin {
    fn build(&self, app: &mut App) {
        app.init_state::<DilationSystemsActive>()
            .add_systems(Update, activate_systems)
            .add_systems(
                Update,
                (DilationTranslation::enact).run_if(in_state(DilationSystemsActive::True)),
            )
...
impl DilationTranslation {
    ...
    fn on_insert(mut world: DeferredWorld, HookContext { entity, .. }: HookContext) {
        ...
        match dilation {
            Some(dilation) => {
                if let Some(mut dilation_translation) =
                    world.entity_mut(entity).get_mut::<DilationTranslation>()
                {
