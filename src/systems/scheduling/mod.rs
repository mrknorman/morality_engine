...
use crate::{
    scenes::{
        dilemma::phases::{consequence::DilemmaConsequenceEvents, intro::DilemmaIntroEvents},
        ending::EndingEvents,
    ...
    fn start_repeat_timer(&mut self) {
        if let Some(repeat_duration_seconds) = self.repeat_duration_seconds {
            if self.just_finished() && self.times_finished == 1 {
                self.timer
                    .set_duration(Duration::from_secs_f32(repeat_duration_seconds));
...
#[derive(Component)]
pub struct TimerPallet<K>(pub EnumMap<K, ConditionalTimer>)
where
