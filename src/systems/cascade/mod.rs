...
use super::{
    colors::{ColorAnchor, ColorExt, OPTION_1_COLOR, OPTION_2_COLOR},
    ...
    fn update(
        ...
        for (children, number) in numbers.iter_mut() {
            ...
            for child in children.iter() {
                let Ok((entity, mut ripple)) = child_query.get_mut(child) else {
                    continue;
                };
            ...
            if let Some(ripples) = by_parent.get(&num_parent.parent()) {
                ...
                for r in ripples {
                    ...
                    let e = (1.0 - t / Ripple::RIPPLE_DURATION)
                        * 0.5
                        * (1.0 + (delta * 0.5 * std::f32::consts::TAU).cos())
                        * (-delta).exp();
                    effect += e;
...
impl Cascade {
    fn update_visibility(
        ...
        numbers
            .par_iter_mut()
            .for_each(|(parent, number, mut visibility)| {
    ...
    fn wrap(
        ...
        numbers
            .par_iter_mut()
            .for_each(|(parent, number, mut transform)| {
    ...
    pub fn enlarge(
        ...
        numbers
            .par_iter_mut()
            .for_each(|(parent, global_transform, mut transform, number)| {
                ...
                if !number.rippling {
                    ...
                    if distance_sq < influence_radius_sq {
                        let scale_factor = if distance_sq <= min_distance_sq {
                            ...
                            let t =
                                (influence_radius - distance) / (influence_radius - min_distance);
    ...
    fn on_insert(mut world: DeferredWorld, HookContext { entity, .. }: HookContext) {
        ...
        let Some(cascade) = entity_ref.get::<Cascade>().cloned() else {
            return;
        };
        ...
        let text_font = entity_ref
            .get::<TextFont>()
            .cloned()
            .unwrap_or_else(|| TextFont {
                font_size: scaled_font_size(10.0),
                ..default()
            });
        ...
        // Generate all configurations in one pass
        {
            ...
            for col in 0..grid_width {
                for row in 0..grid_height {
                    ...
                    let noise_pos = [col as f64 * noise_scale, row as f64 * noise_scale, 0.0];
                    ...
                    cascade_configs.push((Vec3::new(x, y, 0.0), digit, noise_pos));
        ...
        commands.entity(entity).with_children(|parent| {
            for ((position, digit, noise_pos), random_height, visibility) in cascade_configs
                .into_iter()
                .zip(random_heights)
