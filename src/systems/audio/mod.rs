...
impl Plugin for AudioPlugin {
    fn build(&self, app: &mut App) {
        app.init_state::<AudioSystemsActive>()
            ...
            .insert_resource(MusicAudioConfig::new(1.0))
            .insert_resource(NarrationAudioConfig::new(1.0))
            .add_systems(Update, activate_systems)
            .add_systems(
                Update,
                (NarrationAudio::check_if_finished, TransientAudio::tick)
                    .run_if(in_state(AudioSystemsActive::True)),
            )
            .add_systems(
                Update,
                (DilatableAudio::dilate).run_if(resource_changed::<Dilation>),
            );
    }
}
...
impl TransientAudio {
    ...
    pub fn tick(time: Res<Time<Real>>, mut audio_query: Query<&mut TransientAudio>) {
...
pub struct ContinuousAudioPallet<T>
where
    T: Enum + EnumArray<Entity> + Send + Sync + Clone,
    <T as EnumArray<Entity>>::Array: Send + Sync + Clone,
{
...
impl<T> ContinuousAudioPallet<T>
where
    T: Enum + EnumArray<Entity> + Send + Sync + Clone + 'static,
    <T as EnumArray<Entity>>::Array: Send + Sync + Clone,
{
    pub fn new(components: Vec<ContinuousAudio<T>>) -> ContinuousAudioPallet<T> {
...
pub struct TransientAudioPallet<T>
where
...
impl<T> TransientAudioPallet<T>
where
    T: Enum + EnumArray<Vec<Entity>> + Send + Sync + Clone,
    <T as EnumArray<Vec<Entity>>>::Array: Send + Sync + Clone,
{
    pub fn new(components: Vec<(T, Vec<TransientAudio>)>) -> Self {
    ...
    fn on_insert(mut world: DeferredWorld, HookContext { entity, .. }: HookContext) {
        ...
        let components = {
            let mut entity_mut = world.entity_mut(entity);
            entity_mut
                .get_mut::<TransientAudioPallet<T>>()
                .map(|pallet| pallet.components.clone())
        };
                ...
                .with_children(|parent: &mut ChildSpawnerCommands<'_>| {
                    for (key, audio_components) in components.iter() {
                        ...
                        for component in audio_components.iter() {
                            if component.dilatable {
                                child_vector
                                    .push(parent.spawn((component.clone(), DilatableAudio)).id());
                            } else {
                                child_vector.push(parent.spawn(component.clone()).id());
                            }
                        }
...
impl OneShotAudioPallet {
    pub fn new(components: Vec<OneShotAudio>) -> Self {
        Self { components }
    }
                    ...
                    commands.entity(entity).with_children(|parent| {
                        let mut entity_commands = parent.spawn((
                            ...
                            PlaybackSettings {
                                ...
                                speed: dilation
                                    .filter(|_| audio_component.dilatable)
                                    .unwrap_or(audio_component.speed),
                                ..default()
                            },
                        ));
...
impl DilatableAudio {
    fn dilate(dilation: Res<Dilation>, mut audio_query: Query<&mut AudioSink, With<Self>>) {
